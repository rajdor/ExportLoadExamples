...
for i in range (len(columns)):
    ...
    if "CHAR" in columns[i][1]:
        ...
        sqltxt = sqltxt + "\n     WHEN " + columns[i][0] + " LIKE '%|%' OR " + columns[i][0] + " LIKE '%\\%' OR " + columns[i][0] + " LIKE '%\"%' OR " + columns[i][0] + " LIKE '%' || CHR(10) || '%' OR " + columns[i][0] + " LIKE '%' || CHR(13) || '%' OR " + columns[i][0] + " = '' THEN \n"
        sqltxt = sqltxt + "'\"' || " 
        sqlcol = sqlcol + " \nREPLACE("
        sqlcol = sqlcol + " \n        REPLACE(" + columns[i][0] + " ," + escapeChar + "," + escapeChar + " || " + escapeChar +")\n"
        sqlcol = sqlcol +          " ," + DoubleQuote    + "," + escapeChar + " || " + DoubleQuote + ")\n"
            ...
            if "CLOB"       in columns[i][1]:
                ...
                if "DATE"      == columns[i][1].strip():
                    ...
                    if "TIMESTMP"  == columns[i][1].strip():
                        sqltxt = sqltxt + "CAST(CASE WHEN " + columns[i][0] + " IS NULL THEN '' ELSE '" + charDel + "' || SUBSTR(CAST(" + columns[i][0] + " AS CHAR(26)),1,10) || 'T' || REPLACE(SUBSTR(CAST(" + columns[i][0] + " AS CHAR(26)),12,8), '.',':') || '.' || SUBSTR(CAST(" + columns[i][0] + " AS CHAR(26)),21,3) || '+10:00' END  AS VARCHAR(29)) AS " + columns[i][0]
                    else:
