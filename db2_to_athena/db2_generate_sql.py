...
for i in range (len(columns)):
    ...
    if "CHAR" in columns[i][1]:
        ...
        sqlcol = sqlcol + "     ," + escapeChar + "," + escapeChar + " || " + escapeChar + ")"
        sqlcol = sqlcol + "   ," + colDel + "," + escapeChar + " || " + colDel +")"
            ...
            if "CLOB"       in columns[i][1]:
                ...
                if "DATE"      == columns[i][1].strip():
                    ...
                    if "TIMESTMP"  == columns[i][1].strip():
                        sqltxt = sqltxt + "CAST(CASE WHEN " + columns[i][0] + " IS NULL THEN '' ELSE '" + charDel + "' || SUBSTR(CAST(" + columns[i][0] + " AS CHAR(26)),1,10) || ' ' || SUBSTR(CAST(" + columns[i][0] + " AS CHAR(26)),12) || '" + charDel + "' END  AS VARCHAR(28)) AS " + columns[i][0]
                    else:
